...
Public transportation is an important tool that connects communities all over the world. It makes travel and social engagement more accessible, creates jobs, eases traffic congestion, and it can be a sustainable alternative to driving. Mapping Your Transit is a project that celebrates your local transit network with fun and interactive maps. Leveraging the power of an open standard format for transit data, the General Transit Feed Specification (GTFS), this project allows users to upload data from any transit network to view an interactive map and download a custom poster of their favorite bus or subway system. 	
